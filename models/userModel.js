...
var createUser = (user, callback) => {
    ...
    db.executeQuery(sql, [user.name, user.email, 0, user.password, user.gender], function(result) {
        callback(result);
    });
};
...
var updateUser = (user, callback) => {
    var sql = "UPDATE users SET name = ?, email = ?, gender = ? WHERE user_id = ?";
    db.executeQuery(sql, [user.name, user.email,  user.gender, user.user_id], function(result) {
        callback(result);
    });
};
...
var updateCustomer = (id, customer, callback) => {
    var sql = "UPDATE users SET name = ?, email = ?, gender = ? WHERE user_id = ?";
    db.executeQuery(sql, [customer.name, customer.email, customer.gender, id], function(result) {
        callback(result);
    });
};
